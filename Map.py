...
class Map:
    ...
    def map_frame_generator(self):
        # board = np.zeros([grid_no, grid_no], dtype=int)
        # board[:, 0] = 1
        ...
        # dfs(start_pos)
        # np.savetxt("./map_config.txt", board, fmt="%d")
        # print(board.shape)

        #! Modify the following line to load different map configurations
        board = np.loadtxt("./map_config_ez.txt", dtype=int)
        assert board.shape == (
            self.grid_size, self.grid_size), f"map config of size {board.shape} does not match grid size {(self.grid_size, self.grid_size)}"
        print(f"Loaded map with shape = {board.shape}")
        return board
    ...
    def get_tile_rects(self, map_frame, tile_number):
        ...
        for row in range(len(map_frame)):
            ...
            for w in walls:
                res.append(ColliderUtils.generate_block_vertice(
                    row, row, w[0], w[1]))
    ...
    def generate_map(self):
        ...
        self.collider_lines = ColliderUtils.collider_lines_from_path_rects(
            self.path_rects)
    ...
    def draw_map_bg(self):
        ...
        for l in self.collider_lines:
            ImageDraw.Draw(map_image).line(
                [tuple(l[0]), tuple(l[1])], fill=(0, 0, 255), width=2)
            # map_image.show()
        return map_image
