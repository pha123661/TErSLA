...
class ColliderUtils:
    ...
    def radar_pos(vertice):
        ...
        return np.concatenate(
            (r1.reshape(1, 2), r2.reshape(1, 2), r3.reshape(1, 2), r4.reshape(1, 2), r5.reshape(1, 2)))
    ...
    def collider_lines_from_path_rects(path_rects):
        ...
        def merge_lines(l1, l2, axis):
            ...
            if 1 == axis:
                ...
                return np.array([[start, anker], [end, anker]])
        ...
        def check_have_path(start, end, anker, step):
            tmp_path = list()
            for mid in range(start, end, step):
                if mid == end - step:
                    tmp_path.append([[start, anker], [end, anker]])
                    return tmp_path
                if not in_path_rects([[mid, anker], [mid + step, anker]]):
                    continue
                else:
                    tmp_path.append([[start, anker], [mid, anker]])
                    for mid2 in range(mid, end, step):
                        if in_path_rects([[mid2, anker], [mid2 + step, anker]]):
                            continue
                        else:
                            # tmp_path.append([[mid2, anker], [end, anker]])
                            return tmp_path + check_have_path(mid2, end, anker, step)
                    return tmp_path
        ...
        def filter_horizontal_lines(h_lines):
            ...
            for l in h_lines:
                ...
                while 1:
                    ...
                    break
                if start < end:
                    tmp += check_have_path(start, end, anker, step)
...
class ImageUtils:
    ...
    def draw_radar(image, radars, angle, collider_lines):
        angle = np.array(
            [-angle + math.pi / 2, -angle + math.pi / 4, -angle, -angle - math.pi / 4, -angle - math.pi / 2])
